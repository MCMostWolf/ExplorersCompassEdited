...
public class ConcentricRingsSearchWorker extends StructureSearchWorker<ConcentricRingsStructurePlacement> {
	...
	public ConcentricRingsSearchWorker(ServerLevel level, Player player, ItemStack stack, BlockPos startPos, ConcentricRingsStructurePlacement placement, List<ConfiguredStructureFeature<?, ?>> configuredStructureSet, String managerId, List<Pair<UUID, ChunkPos>> foundChunks) {
		super(level, player, stack, startPos, placement, configuredStructureSet, managerId, foundChunks);
	...
	public boolean doWork() {
		...
		if (!foundChunks.contains(Pair.of(player.getUUID(), new ChunkPos(currentPos)))) {
			if (closest != null) {
