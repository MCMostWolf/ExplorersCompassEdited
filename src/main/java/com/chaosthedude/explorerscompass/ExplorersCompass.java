...
public class ExplorersCompass {
	...
	private void commonSetup(FMLCommonSetupEvent event) {
		...
		// Server packets
		network.registerMessage(i, CompassSearchPacket.class, CompassSearchPacket::toBytes, CompassSearchPacket::new, CompassSearchPacket::handle);
        i++;
		network.registerMessage(i, TeleportPacket.class, TeleportPacket::toBytes, TeleportPacket::new, TeleportPacket::handle);
        i++;
        network.registerMessage(i, NewWayPointPacket.class, NewWayPointPacket::toBytes, NewWayPointPacket::new, NewWayPointPacket::handle);


		// Client packet
        i++;
		network.registerMessage(i, SyncPacket.class, SyncPacket::toBytes, SyncPacket::new, SyncPacket::handle);
        i++;
		network.registerMessage(i, CleanFoundPacket.class, CleanFoundPacket::toBytes, CleanFoundPacket::new, CleanFoundPacket::handle);
	...
	public void clientSetup(FMLClientSetupEvent event) {
		...
		event.enqueueWork(() -> ItemProperties.register(explorersCompass, new ResourceLocation("angle"), new ClampedItemPropertyFunction() {
            @OnlyIn(Dist.CLIENT)
            ...
            public float unclampedCall(@NotNull ItemStack stack, ClientLevel world, LivingEntity entityLiving, int seed) {
                if (entityLiving == null && !stack.isFramed()) {
                    ...
                    if (entity != null && world == null && entity.level() instanceof ClientLevel) {
